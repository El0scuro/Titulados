  ...
  imports: [
    ...
    TypeOrmModule.forRootAsync({
      ...
      useFactory: async (configService: ConfigService): Promise<TypeOrmModuleOptions> => ({
        type: 'mysql',
        host: configService.get<string>('DB_HOST') || 'localhost' || 'db',
        port: parseInt(configService.get<string>('DB_PORT')) || 3310,
        username: configService.get<string>('DB_USERNAME') || 'root',
        password: configService.get<string>('DB_PASSWORD') || 'transporteSW0608',
        database: configService.get<string>('DB_NAME') || 'tituladosv2',
